...
class EnrichCitations_datestamp():
    def __init__(
        self,
    ...
    def get_output_path(self, organism):
        return f"{self.output_dir}/{organism}-pubmed-citations_date_stamp.tsv"
    ...
    def get_cites_by_gene(self, organism, pmid_dates_path):
        ...
        for gene_id in pmids_by_gene:
            gene_id_date = str(gene_id) + str('-date')
            cites_by_gene[gene_id] = len(pmids_by_gene[gene_id])
            cites_by_gene[gene_id_date] = pmids_by_gene[gene_id]
        # print(cites_by_gene)
    ...
    def run(
        ...
        for org in organisms:
            ...
            cite_ranks = rank_counts(cites_by_gene)
            prev_cite_ranks = rank_counts(prev_cites_by_gene)

            if len(prev_cite_ranks) == 0:
                print(
                    f"No publications in PubMed cited genes from {organism} " +
                    f"between last {days} and {days*2} days.  " +
                    f"Try increasing the `days` value in `citations.py`."
                )
                continue
